...
class Room:
    ...
    price_ = {'одноместный': 2900.00, 'двухместный': 2300.00, 'люкс': 4100.00, 'полулюкс': 3200.00}
    grow_price = {'стандарт': 1.0, 'стандарт_улучшенный': 1.2, 'апартамент': 1.5}
    ...
    def __price(self, budget, discount):
        ...
        price_ = {'одноместный': 2900., 'двухместный': 2300., 'люкс': 4100., 'полулюкс': 3200.}
        grow_price = {'стандарт': 1., 'стандарт_улучшенный': 1.2, 'апартамент': 1.5}
        ...
        return pay  # макс цена по бюджету клиента
    ...
    def check_reserve(self, date):
        ...
        reservation = datetime.date(date_year, date_month, date_day)  # дата бронирования
        exit = datetime.date(self.date_year, self.date_month, self.date_day)  # дата освобождения
...
class Hotel:
    ...
    def bookings(self, file='booking.txt'):
        with open('booking.txt', encoding='utf8') as f:
            ...
            for i, k in enumerate(text):
                fio = k.split()  # 01.03.2018 Аксёнов Лаврентий Семенович 3 03.03.2018 2 4600
                date_reserve = fio[0]
                name = fio[1] + ' ' + fio[2] + ' ' + fio[3]
                ...
                if ret:
                    ...
                    print('номер', right.number, right.type_, right.comfort, 'рассчитан на',
                          ...
                          right.food_type, 'стоимость', current_price, 'руб./сутки\n')
                    if self.agree():
    ...
    def print_status(self, date):
        ...
        percent = len(stockpile) * 100 / len(self.rooms)

        print('Количество занятых номеров:', len(stockpile), '\n')
